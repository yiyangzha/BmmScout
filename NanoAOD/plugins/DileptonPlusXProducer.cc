...
private:
  ...
  void fillDstarInfo(pat::CompositeCandidateCollection &dstar_collection,
                     const edm::Event &iEvent,
...
DileptonPlusXProducer::DileptonPlusXProducer(const edm::ParameterSet &iConfig) : beamSpotToken_(consumes<reco::BeamSpot>(iConfig.getParameter<edm::InputTag>("beamSpot"))),
                                                                                 beamSpot_(nullptr),
                                                         ...
                                                         std::vector<const pat::PackedCandidate *> ignoreTracks)
{
  ...
  {
    if (not isGoodTrack(pfCand))
      continue;
    if (overlap(lepton1, pfCand) || overlap(lepton2, pfCand))
      continue;
    ...
    double mu2_kaon_doca = distanceOfClosestApproach(lepton2.track(),
                                                     pfCand.bestTrack());
    if (mu1_kaon_doca > maxDoca or mu2_kaon_doca > maxDoca)
      continue;
...
float DileptonPlusXProducer::computeTrkLeptonIsolation(const bmm::Candidate &theLepton,
                                                       ...
                                                       std::vector<const pat::PackedCandidate *> ignoreTracks)
{
  ...
  for (const auto &pfCand : *pfCandHandle_.product())
  {
    ...
    if (pfCand.pt() < minPt)
      continue;
    if (pfCand.vertexRef().key() != primaryVertexIndex)
      continue;
    if (overlap(theLepton, pfCand) || overlap(theOtherLepton, pfCand))
      continue;
    if (deltaR(theLepton, pfCand) > dR)
      continue;
    sumPt += pfCand.pt();
  }
...
float DileptonPlusXProducer::computeTrkDileptonIsolation(const bmm::Candidate &lepton1,
                                                         const bmm::Candidate &lepton2,
                                                         unsigned int primaryVertexIndex,
                                                         float minPt, float dR,
                                                         std::vector<const pat::PackedCandidate *> ignoreTracks)
...
float DileptonPlusXProducer::otherVertexMaxProb(const bmm::Candidate &lepton1,
                                                const bmm::Candidate &lepton2,
                                                float minPt,
                                                float max_doca,
                                                std::vector<const pat::PackedCandidate *> ignoreTracks)
                                        ...
                                        const bmm::Candidate &muon3)
{
  ...
  if (muon1.track())
    result.tracks.push_back(muon1.track());
  if (muon2.track())
    result.tracks.push_back(muon2.track());
  if (muon3.track())
    result.tracks.push_back(muon3.track());
...
void DileptonPlusXProducer::fillDstarInfo(pat::CompositeCandidateCollection &dstar_collection,
                                          const edm::Event &iEvent,
                                          const KinematicFitResult &d0VertexFit,
                                          const pat::CompositeCandidate &d0Cand,
                                          const pat::PackedCandidate &soft_pion,
                                          ...
                                          int fromKpi)
{
  ...
  reco::TransientTrack softPionTT = (*theTTBuilder_).build(soft_pion.bestTrack());
...
void DileptonPlusXProducer::fillMvaInfoForBtoJpsiKCandidatesEmulatingBmm(pat::CompositeCandidate &llK,
                                                                         const pat::CompositeCandidate &ll,
                                                                         const edm::Event &iEvent,
                                                                         const KinematicFitResult &kinematicLLVertexFit,
                                                                         const bmm::Candidate &lepton1,
...
void DileptonPlusXProducer::fillMvaInfoForLLGamma(pat::CompositeCandidate &llg,
                                                  const pat::CompositeCandidate &ll,
                                                  const edm::Event &iEvent,
                                                  const KinematicFitResult &kinematicLLVertexFit,
                                                  const bmm::Candidate &lepton1,
...
void DileptonPlusXProducer::fillIsolationInfo(const pat::CompositeCandidate &refCand,
                                              ...
                                              const reco::Track *refTrack2)
{
  ...
  for (const auto &pfCand : *pfCandHandle_.product())
  {
    ...
    if (overlap(refTrack1, track) || overlap(refTrack2, track))
      continue;
    ...
    if (track_beamspot_ip_significance > 2.0)
    {
      ...
      if (track->pt() > 1 and dR < 1.5 and
          (d1_doca < maxDoca or d2_doca < maxDoca))
        save_track = true;
    ...
    if (dR < 1.5 and d1_doca < maxTwoTrackDOCA_ and d2_doca < maxTwoTrackDOCA_)
    {
      ...
      if (ddTrkVertexFit.vtxProb() > 0.001)
        save_track = true;
    }
...
void DileptonPlusXProducer::fillTrackInfo(pat::CompositeCandidateCollection &output_collection,
                                          const std::vector<const reco::Track *> &tracks,
                                          const edm::Event &iEvent)
{
...
void DileptonPlusXProducer::injectHadronsThatMayFakeMuonsInMC(std::vector<bmm::Candidate> &good_lepton_candidates)
{
  ...
  for (auto const &cand : *prunedGenParticles_)
  {
    ...
    for (const auto &pfCand : *pfCandHandle_.product())
    {
      if (pfCand.charge() == 0)
        continue;
      if (not pfCand.hasTrackDetails())
        continue;
      for (auto hadron : final_state_particles)
      {
        if (deltaR(*hadron, pfCand) > 0.01)
          continue;
...
void DileptonPlusXProducer::injectBhhHadrons(std::vector<bmm::Candidate> &good_lepton_candidates)
{
  ...
  for (const auto &pfCand : *pfCandHandle_.product())
  {
    if (not isGoodTrack(pfCand))
      continue;
    if (pfCand.pt() < minBhhTrkPt_)
      continue;
    if (abs(pfCand.eta()) > maxBhhTrkEta_)
      continue;
...
void DileptonPlusXProducer::injectJpsiTracks(std::vector<bmm::Candidate> &good_lepton_candidates)
{
  ...
  {
    if (not isGoodMuonCandidateFromTrack(pf_candidates.at(i)))
      continue;
    bool new_mu = true;
    for (const auto &lep : muon_candidates)
    {
      if (overlap(lep, pf_candidates.at(i)))
        new_mu = false;
    }
    if (not new_mu)
      continue;
  ...
  if (muon_candidates.size() - good_lepton_candidates.size() < 2)
    return;
  ...
  for (unsigned int i = 0; i < muon_candidates.size() - 1; ++i)
  {
    ...
    {
      if (muon_candidates.at(i).charge() == muon_candidates.at(j).charge())
        continue;
      if (fabs((muon_candidates.at(i).p4() + muon_candidates.at(j).p4()).mass() - 3.1) > 0.2)
        continue;
      ...
      {
        if (overlap(lep, muon_candidates.at(i)))
          new_mu1 = false;
        if (overlap(lep, muon_candidates.at(j)))
          new_mu2 = false;
      }
...
void DileptonPlusXProducer::buildLLXCandidates(pat::CompositeCandidateCollection &llk,
                                               pat::CompositeCandidateCollection &llkk,
                                               const edm::Event &iEvent,
                                               const KinematicFitResult &kinematicLLVertexFit,
                                               const pat::CompositeCandidate &dileptonCand,
                                               ...
                                               const bmm::Candidate &lepton2)
{
  ...
  for (unsigned int k = 0; k < nPFCands; ++k)
  {
    ...
    if (not isGoodTrack(kaonCand1))
      continue;
    if (abs(kaonCand1.pdgId()) != 211)
      continue;
    if (kaonCand1.pt() < ptMinKaon_ or abs(kaonCand1.eta()) > etaMaxKaon_)
      continue;
    ...
    for (unsigned int k2 = k + 1; k2 < nPFCands; ++k2)
    {
      ...
      if (not isGoodTrack(kaonCand2))
        continue;
      if (abs(kaonCand2.pdgId()) != 211)
        continue;
      if (kaonCand2.pt() < ptMinKaon_ || abs(kaonCand2.eta()) > etaMaxKaon_)
        continue;
      ...
      for (unsigned int j = 0; j < good_muon_candidates.size(); ++j)
      {
          ...
          for (unsigned int k = 0; k < nPFCands; ++k)
          {
                ...
                (dstar_mass - d0_mass) > min_dm_ && (dstar_mass - d0_mass) < max_dm_)
            {
              ...
              fillDstarInfo(*dstar_collection, iEvent, kinematicLLVertexFit, dimuonCand,
                            soft_pion, mm_index, -1, muon1, muon2, 0);
            }
          }
        }*/
        ...
        if (recoKstar_)
        {
          for (unsigned int k = 0; k < nPFCands; ++k)
          {
            ...
            if (not isGoodHadron(pion))
              continue;

            if (overlap(muon1, pion) || overlap(muon2, pion))
              continue;
  ...
  if (recoElElX_ && good_electron_candidates.size() > 1)
  {
    for (unsigned int i = 0; i < good_electron_candidates.size(); ++i)
    {
      ...
      for (unsigned int j = 0; j < good_electron_candidates.size(); ++j)
      {
        ...
        if (not preprocess(dielectronCand, iEvent, electron1, electron2))
          continue;
  ...
  if (recoElMu_)
  {
    for (unsigned int i = 0; i < good_electron_candidates.size(); ++i)
    {
      ...
      for (unsigned int j = 0; j < good_muon_candidates.size(); ++j)
      {
        ...
        if (not preprocess(emuCand, iEvent, electron, muon))
          continue;
      ...
      for (unsigned int j = i + 1; j < nPFCands; ++j)
      {
        ...
        /*if (recoKstar_)
        {
          for (unsigned int k = 0; k < nPFCands; ++k)
          {
            ...
            if (overlap(had1, pion) || overlap(had2, pion))
              continue;
  ...
  if (nPFCands > 3)
  {
    for (unsigned int ihad1 = 0; ihad1 < nPFCands - 1; ++ihad1)
    {
      ...
      for (unsigned int ihad2 = ihad1 + 1; ihad2 < nPFCands; ++ihad2)
      {
        ...
        if (ihad1 < nPFCands - 2)
        {
          for (unsigned int ihad3 = ihad1 + 1; ihad3 < nPFCands - 1; ++ihad3)
          {
            ...
            for (unsigned int ihad4 = ihad3 + 1; ihad4 < nPFCands; ++ihad4)
            {
              ...
              if (had3.charge() == had4.charge())
                continue;

              if (fabs((had3.p4() + had4.p4()).mass() - 1.02) > 0.01)
                continue;
                                 ...
                                 float mass_constraint)
{
  ...
  if (lepton2.track())
    result.tracks.push_back(lepton2.track());
  if (kaon.bestTrack())
    result.tracks.push_back(kaon.bestTrack());
                                  ...
                                  float kk_mass_constraint)
{
  ...
  if (lepton2.track())
    result.tracks.push_back(lepton2.track());
  if (kaon1.bestTrack())
    result.tracks.push_back(kaon1.bestTrack());
  if (kaon2.bestTrack())
    result.tracks.push_back(kaon2.bestTrack());
...
int DileptonPlusXProducer::nanoGenParticle(const reco::Candidate *p)
{
  ...
  if (gen)
  {
    ...
    for (unsigned int i = 0; i < nanoGenParticles_->size(); ++i)
    {
      if (nanoGenParticles_->at(i).pdgId() != gen->pdgId())
        continue;
      if (deltaR(nanoGenParticles_->at(i).p4(), gen->p4()) > 0.01)
        continue;
      if (fabs(nanoGenParticles_->at(i).pt() - gen->pt()) / gen->pt() > 0.01)
        continue;
      return i;
    }
  }
                                                    ...
                                                    const bmm::Candidate *lepton3)
{
  ...
  if (result.mc_l1 and result.mc_l2)
  {
    ...
    if (result.mc_l1->mother() and result.mc_l1->mother() == result.mc_l2->mother())
    {
      ...
      if (ll_mother->mother())
        result.ll_motherPdgId = ll_mother->mother()->pdgId();
...
DileptonPlusXProducer::getGoodTracksToRefitPV(int pvIndex, std::vector<T> ignoreTracks)
{
  ...
  for (const auto &pfCand : *pfCandHandle_.product())
  {
    if (not isGoodTrack(pfCand))
      continue;
    if (int(pfCand.vertexRef().key()) != pvIndex)
      continue;
    // keep only the tracks used in the PV fit
    if (pfCand.pvAssociationQuality() != pat::PackedCandidate::UsedInFitTight)
      continue;
...
float DileptonPlusXProducer::computeAnalysisBDT(unsigned int event_idx)
{
  switch (event_idx)
  {
  case 0:
